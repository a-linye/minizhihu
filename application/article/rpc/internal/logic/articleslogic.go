...
import (
	...
	"minizhihu/application/article/rpc/internal/code"
	"minizhihu/application/article/rpc/internal/model"
	"minizhihu/application/article/rpc/internal/types"
	"slices"
	"strconv"
	"time"
...
func (l *ArticlesLogic) Articles(in *pb.ArticlesRequest) (*pb.ArticlesResponse, error) {
	...
	if len(articleIds) > 0 {
		...
		// 缓存中的文章的顺序，可能和数据库的数据顺序不一致，需要根据数据库的数据再排序
		...
		v, err, _ := l.svcCtx.SingleFlightGroup.Do(fmt.Sprintf("ArticlesByUserId:%d:%d", in.UserId, in.SortType), func() (interface{}, error) {
			return l.svcCtx.ArticleModelExtra.ArticlesByUserId(l.ctx, in.UserId, types.ArticleStatusVisible, sortLikeNum, sortPublishTime, sortField, types.DefaultLimit)
		})
		if err != nil {
...
func (l *ArticlesLogic) articleByIds(ctx context.Context, articleIds []int64) ([]*model.Article, error) {
	articles, err := mr.MapReduce[int64, *model.Article, []*model.Article](func(source chan<- int64) {
		...
		p, err := l.svcCtx.ArticleModelExtra.FindOne(ctx, id)
		if err != nil {
			cancel(err)
			return
		}
